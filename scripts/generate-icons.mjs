 ...
 * an off-white C and a sky-blue B on the site's dark page colour. (Redrawn
 * 2026-09-09 — the previous white-on-blue rounded square read as LinkedIn's
 * logo.) The letters are pre-outlined vector paths (extracted from
...
/** Tile: the site's dark page colour — keep in sync with --page (dark) in global.css. */
const TILE = '#16171a';
/** The C: dark-mode --ink. */
const INK = '#f0eee9';
/** The B: dark-mode --pine (link blue). */
const SKY = '#6fb2ef';
...
// The path is two glyphs: everything up to the first Z is the C, the rest is
// the B (with its two counters). Split so each can take its own colour.
const C_PATH = MARK_PATH.slice(0, MARK_PATH.indexOf('Z') + 1);
const B_PATH = MARK_PATH.slice(MARK_PATH.indexOf('Z') + 1);
const markPaths = `<path fill="${INK}" d="${C_PATH}"/><path fill="${SKY}" d="${B_PATH}"/>`;
...
function iconSvg({ radius, inset = 1 }) {
  const mark =
    inset === 1
      ? markPaths
      : `<g transform="translate(16 16) scale(${inset}) translate(-16 -16)">${markPaths}</g>`;
  return (
    `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32" width="32" height="32" ` +
    `role="img" aria-label="Cory Bergman">` +
    `<rect width="32" height="32"${radius ? ` rx="${radius}"` : ''} fill="${TILE}"/>` +
    mark +
    `</svg>`
  );
}
