...
- Avatar (circular headshot, `src/assets/avatar-headshot.jpg`): `src/components/Avatar.astro` — a 512×512 square crop of the *same* professional headshot the About page shows (`corybergman_photo.png`), so the face matches site-wide. The **404 page deliberately keeps the older candid shot** (`src/assets/avatar.jpg`), which is now used nowhere else — the mismatch is the joke there, so don't "fix" it.
