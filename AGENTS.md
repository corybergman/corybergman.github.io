...
- Masthead photo (tall/short + per-page override): `src/components/TopoMasthead.astro`. `variant="tall"` → `masthead1.png` (homepage), `variant="short"` → `masthead2.png` (everywhere else). A page can override both by passing `masthead={…}` to `BaseLayout`, which forwards it as TopoMasthead's `image` prop — **About does this** with `src/assets/masthead-about.jpg` (the Seattle skyline) and **Projects does too** with `src/assets/masthead-projects.jpg` (the Pike Place Market sign at dusk; band `top:535, height:444` at full width of Cory's 2000×1500 `src/assets/pikeplace.png`, resized to 1800×400, mozjpeg q88 — this is the second photo, he rejected the first daytime one), and **Speaking** with `src/assets/masthead-speaking.jpg` (Seattle waterfront with a state ferry and the skyline; band `top:0, height:444` of Cory's 2000×465 `src/assets/waterfront.jpg`, resized to 1800×400, mozjpeg q88 — replaced an Amazon HQ/Spheres shot he rejected the same day. Cory's rule for this one: the Smith Tower spire at the top and the ferry hull at the bottom must both stay in frame, so only trim water). Supply overrides at **1800×400** (2× the 900×200 display size) or they look soft on retina; the 4.5:1 ratio matches the other two.
...
- Headshots: `src/assets/corybe_2026.jpg` is the 2026 studio portrait **already cropped by Cory to head and shoulders (1173×1012, no hands)** — he re-uploaded it via the GitHub web UI on 2026-09-09, replacing the full-length 1173×1600 original. The home hero shows it via `astro:assets` (`widths={[295, 590]}`) in a 250×225 rounded box with `object-fit: cover; object-position: 50% 8%`. **On phones (≤700px)** a hand-written `<picture>` swaps in `headshot-2026.jpg` as a 112px square beside the H1 (the hero becomes a grid, `.hero-text` goes `display: contents`) — Astro's `<Picture>` has no slot for art-direction sources, hence the manual markup via `getImage`. About uses `src/assets/headshot-2026.jpg`, an 800×800 face crop (sharp `extract({left:230, top:80, width:660, height:660})` — the coordinates still hold, since the new file is the old one with the bottom removed). The About "Download headshot" link serves `public/corybergman.jpg`, a straight mozjpeg re-encode of `corybe_2026.jpg` with metadata stripped — regenerate it with sharp whenever that asset changes. The OG-card avatar in `src/og/card.ts` reads `headshot-2026.jpg`. The old candid selfie `src/assets/avatar.jpg` is used **only by the 404 page** (the joke depends on that face); don't swap it there. `Avatar.astro` was deleted with the header avatar.
- Post layout (date, H1, body, tags, prev/all nav): `src/layouts/BlogPost.astro`
- Home page: `src/pages/index.astro` — **redesigned 2026-09-08** from the Claude Design handoff at `~/Documents/design_handoff_corybe_home/` (README = spec; `Corybe Home.dc.html` = reference). Top to bottom: hero (H1 "Trusted information for people in harm's way.", lede, Factal line, "More about me →", portrait), **Where I focus** (three `auto-fit` columns, each a sentence + link; the BUILD/VERIFY/APPLY labels and per-column headings were deliberately removed in design — don't reintroduce), the **Speaking card** (the only enclosed block: eyebrow, pitch, body and "See upcoming speaking events →" → `/speaking`; it was designed as two columns with an "Upcoming: Global Security Briefing" half on the right, which Cory dropped on 2026-09-08 for simplicity — don't reintroduce it), then **Writing**: the newest `RECENT_COUNT` (5) posts as date / title / one-line excerpt, then "Browse the full archive, back to 2011 →" → `/writing/`. **Excerpts are the post's `description` frontmatter**, so keep new descriptions to roughly 100–120 characters — they double as the meta description and RSS teaser. The middle focus link is "More about verification →" → `/tags/verification/`; the `verification` tag was created 2026-09-09 and applied to the four posts about establishing what's true (AI verification, disinformation, fact-checking) — tag future posts on that theme with it so the link stays fresh. All hero/focus/speaking copy was written by Cory — do not reword. · About: `src/pages/about.astro`
- **No archive toggle and no pagination on the home page** (since 2026-09-08). The old `<details>` "Earlier writing" toggle went with the redesign; the full list of posts lives on `/writing/` (grouped by year) and each `/tags/<slug>/` archive, and every post keeps its one permanent URL. If a chronological all-posts page is ever wanted again, add it as a new route rather than reviving the toggle.
...
- **Writing page** (`src/pages/writing.astro` → `/writing/`, since 2026-09-09): the archive. Intro line, topic chips as the filter row (most-used first, linking to `/tags/<slug>/`), then every post newest-first grouped by year in compact date/title rows. It replaced the old `/tags/` topic index, which is now a redirect in `astro.config.mjs` (`redirects: { '/tags': '/writing' }`). The per-topic archives **stay at `/tags/<slug>/`** (`src/pages/tags/[tag].astro`; don't rename them — they're linked from post chips and llms.txt and are indexed); their crumb reads "Writing / <tag>" and they use the `writing` nav key and OG card. Tag chips on posts link here via the `tagSlug()` helper in `src/consts.ts` — always slug tags with it so links and routes stay in sync.
