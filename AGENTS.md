...
- Header + nav (home/sub variants, active state): `src/components/SiteHeader.astro` — the nav is **Home · Topics · Projects · Speaking · About** (RSS moved to the footer), driven by a single `links` array so the home and sub headers can't drift apart. To add a tab: add an entry there, widen the exported `NavKey` union, and pass the matching `active="…"` from the page. `NavKey` is re-used by `BaseLayout`, so a typo in `active` is a type error. At ≤700px the nav drops to 13px/13px-gap so the links still fit one row on a 375px phone.
...
- Avatar (circular headshot, `src/assets/headshot-2026.jpg`): `src/components/Avatar.astro` — used in the site header on every page. Since 2026-09-08 the header avatar and the About-page headshot are the **same** square crop of the 2026 studio portrait, so the two stay aligned; change one and change both. `headshot-2026.jpg` is an 800×800 cut from the full portrait `src/assets/corybe_2026.jpg` (sharp `extract({left:230, top:80, width:660, height:660})`, mozjpeg q86) — keep the full portrait in the repo for re-cropping. The About page's "Download headshot" link serves `public/corybergman.jpg`, which is the **full** 2026 portrait (1173×1600, re-encoded with metadata stripped), and the OG-card avatar in `src/og/card.ts` reads `headshot-2026.jpg` too — so the face is the same crop everywhere except the 404. The old candid selfie `src/assets/avatar.jpg` is now used **only by the 404 page** (the joke depends on that face); don't swap it there.
